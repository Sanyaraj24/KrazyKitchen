...
app.get("/recipeStream", (req, res) => {
  const { ingredients, cuisine, cookingTime, complexity, people, note } =
    req.query;
  ...
  const customNote = note
    ? `Please ensure the recipe meets the following requirement: ${note}.`
    : "";
  ...
  const messages = [
    {
      role: "system",
      content:
        "You are a recipe assistant that provides structured recipes in plain text format with bold headings, including a nutritional summary per serving.",
    },
